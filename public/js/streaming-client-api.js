...
async function initAPI() {
  try {
      ...
      console.log('DID_API:', DID_API); // Vérifiez que la clé API est chargée
...
/*// Chargement asynchrone du fichier api.json
async function initAPI() {
    try {
        const response = await fetch('/api.json');
        if (!response.ok) {
        ...
        // Ici vous pouvez initialiser le reste de votre application
        // qui dépend de DID_API
  ...
  try {
    ...
    const requestBody = {
      script: {
        type: 'text',
        subtitles: 'false',
        provider: { type: 'microsoft', voice_id: 'fr-FR-RemyMultilingualNeural' },
        ssml: false,
